...
epistats5 <- build_epistats(geog.lvl = "state", geog.cat = "WA", race = FALSE)
```
...
A further addition to the ARTnet workflow is allowing for age group subsetting and arbitrary setting of age groups. Currently ARTnet uses the full range of ages within ARTnetData and age groups are set at `0, 24, 34, 44, 54, 64, 100`. This is retained as the default behavior behavior of ARTnet with new variables `age.lim` in `build_epistats` and `age.bks` and `age.cat` allowing for age range and group specification respectively:
...
* `age.cat`: specifies total number of age categories to use; used only if `age.bks` is not specified.
...
netparams1 <- build_netparams(epistats = epistats6, age.bks = c(20, 30, 40, 50, 60), smooth.main.dur.55p = TRUE)

#Specify the number of age categories
netparams2 <- build_netparams(epistats = epistats6, age.cat = 6, smooth.main.dur.55p = TRUE)

#Specify neither; build_netparams will default to 6
netparams3 <- build_netparams(epistats = epistats6, smooth.main.dur.55p = TRUE)
```
