...
When `geog.lvl` is non-null, every joint model above also includes `geogYN` as a main-effect covariate, with predictions in `build_netstats()` taken at `geogYN = 1` — the same mechanism the legacy method uses to specialize the regression to the focal city while borrowing strength across the full sample, layered here on top of the joint multivariate adjustment.
...
The target-statistic shifts in Tables 3 and 4 conflate two distinct effects. The first, a *confounding-adjustment* effect, is the regression-level change in per-cell rates that emerges when the joint multivariate fit replaces the existing method's univariate fits, holding the population fixed at ARTnet's sample. The second, a *population-shift* effect, is the further change in those per-cell rates when the joint regression is evaluated at the synthetic target's joint attribute distribution rather than ARTnet's. The two compose into the final per-capita rate produced by `build_netparams` + `build_netstats`; the target statistic is then per-capita × synth cell count. Table 6 isolates the two components for the main-layer formation per-capita stats on the Atlanta default scenario, by evaluating the joint Poisson at two reference distributions: ARTnet's Atlanta sub-sample (`geogYN = 1`, n = 206), which preserves ARTnet's within-cell joint distribution of the other attributes (the "Joint @ ARTnet" column), and the Atlanta synth (N = 5000), the actual aggregation target ("Joint @ synth").

**Table 6.** Per-capita main-layer rate decomposition, atlanta_default scenario. `Existing` is the legacy method's per-cell prediction at `geogYN = 1`. `Joint @ ARTnet` is the joint Poisson's prediction averaged within each cell over ARTnet's Atlanta sub-sample; the `%Δ_conf` column is its signed shift from `Existing`. `Joint @ synth` is the joint Poisson averaged within each cell over the Atlanta synth; `%Δ_pop` is its shift from `Joint @ ARTnet`, and `%Δ_total` its shift from `Existing`.

| Stat | Level | Existing | Joint @ ARTnet | Joint @ synth | %Δ_conf | %Δ_pop | %Δ_total |
|---|---|---:|---:|---:|---:|---:|---:|
| md.main | overall | 0.398 | 0.398 | 0.338 | 0.0% | −15.1% | −15.1% |
| nf.race | Black (1) | 0.277 | 0.270 | 0.272 | −2.6% | +0.7% | −2.0% |
| nf.race | Hispanic (2) | 0.418 | 0.475 | 0.408 | +13.5% | −14.1% | −2.6% |
| nf.race | W/Other (3) | 0.415 | 0.410 | 0.408 | −1.2% | −0.5% | −1.7% |
| nf.age.grp | 1 | 0.380 | 0.378 | 0.305 | −0.3% | −19.5% | −19.7% |
| nf.age.grp | 2 | 0.477 | 0.495 | 0.412 | +3.7% | −16.7% | −13.7% |
| nf.age.grp | 3 | 0.451 | 0.410 | 0.398 | −9.1% | −2.9% | −11.7% |
| nf.age.grp | 4 | 0.367 | 0.369 | 0.326 | +0.4% | −11.4% | −11.1% |
| nf.age.grp | 5 | 0.272 | 0.286 | 0.236 | +5.0% | −17.2% | −13.1% |
| nf.deg.casl | 0 | 0.444 | 0.450 | 0.381 | +1.1% | −15.3% | −14.3% |
| nf.deg.casl | 1 | 0.351 | 0.329 | 0.292 | −6.3% | −11.3% | −16.9% |
| nf.deg.casl | 2 | 0.276 | 0.285 | 0.227 | +3.2% | −20.3% | −17.7% |
| nf.deg.casl | 3 | 0.218 | 0.240 | 0.183 | +9.8% | −23.7% | −16.2% |
| nf.diag.status | HIV− | 0.393 | 0.392 | 0.334 | −0.5% | −14.6% | −15.0% |
| nf.diag.status | HIV+ | 0.432 | 0.445 | 0.351 | +3.0% | −21.2% | −18.8% |

The overall main degree (`md.main`) shows zero confounding-adjustment effect by construction: the joint Poisson's mean fitted value within `geogYN = 1` equals the empirical mean within `geogYN = 1` by the score equation, so the full −15.1% main-edges shift from Table 4 is the population-shift component alone. At the per-attribute cell level, however, both components can be substantial and can move in opposite directions. The largest confounding adjustments are +13.5% on the Hispanic per-race rate, +9.8% on `deg.casl = 3` (reflecting the joint regression's AIC-selected `age.grp:deg.casl` interaction at +0.10 — Table 5 — which weakens the main-on-casual substitution for older respondents), and −9.1% on the middle-aged stratum (`age.grp = 3`). For Hispanic egos, the +13.5% confounding adjustment is then offset by a −14.1% population shift to give a small −2.6% net change in per-capita rate. The pattern confirms that the population-shift component dominates the aggregate target-stat changes of Table 3, but the confounding-adjustment component is non-negligible at the per-cell level and is the load-bearing piece for cell-level rate corrections at the regression step — exactly the part that an `ergm.ego`-style design-based aggregation of ego sample totals would not deliver, since those sample totals have no mechanism for adjusting per-cell rates to a target population that differs from the sample on within-cell joint attribute structure.
...
**Relation to prior egocentric ERGM methodology.** The most natural existing alternative within the statnet ecosystem is the `ergm.ego` package (Krivitsky and Morris, 2017), which provides design-based estimation of ERGMs from egocentrically sampled network data. For each ERGM term, `ergm.ego` defines a per-ego summary contribution and computes the target statistic as a Horvitz–Thompson weighted sample total scaled to a pseudo-population network of size `|N'|`; the ERGM is then fit jointly across all model terms via MCMC-MLE on that pseudo-population, with a `−log|N'|` size adjustment on the edges term (Krivitsky, Handcock, and Morris, 2011). For the formation terms in scope here — `edges`, `nodefactor`, `nodematch`, `absdiff` — the per-ego sample totals computed by `ergm.ego` are structurally identical to the legacy ARTnet `netstats` fields, and `ergm.ego`'s MCMC-MLE estimator already provides a form of joint inference for the cross-sectional formation parameters in that all coefficients are estimated simultaneously to satisfy all sample totals. The decisive constraint is that `ergm.ego` requires the egocentric sample to be a probability sample of the target population, with the target's joint attribute distribution either matching the sample's or recoverable from the sample via design weights. ARTnet violates this assumption for city-specific applications: the dataset is a national online-recruited convenience sample with known race-selection bias operating within geographic strata, and the city-specific MSM targets differ from the ARTnet sample on the joint distribution of race × age. The available `ergm.ego` adjustment for this discrepancy — inverse-probability weighting through the `egor` survey design — is unstable in regimes where the target's marginal cells have very low ego support (Atlanta's 51.5% Black target against ARTnet's 5.5% Black sample implies a 9.4× weight on a cell of approximately 270 Black respondents in the full sample, and far fewer in any race × age × geography stratum). This is the standard failure mode of inverse-probability weighting under heavy covariate shift, and it motivates regression-based g-computation as the more stable alternative; the joint refactor presented here can be read as the g-computation counterpart to the IPW estimator that an `ergm.ego` workflow would imply for this setting. Beyond the formation-stat scope, `ergm.ego` is cross-sectional only and has no machinery for the partnership-duration estimand discussed above, nor for the truncated alter-roster design used to define the ARTnet `inst` (one-time-partnership) layer.
...
- Krivitsky PN, Handcock MS, Morris M. Adjusting for Network Size and Composition Effects in Exponential-Family Random Graph Models. *Statistical Methodology* 2011; 8(4): 319–339.
