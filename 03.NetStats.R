...
# W = White, Asian
rdist <- as.data.frame(read_excel("inst/RaceDistribution.xlsx"))
...
# diag status
xs <- data.frame(age = attr_age, race2 = attr_race, cityYN = 1)
...
out$main$nodefactor_age.grp <- unname(table(out$attr$age.grp)) * nstats$main$nf.age.grp
...
out$main$nodefactor_deg.casl <- num * nstats$main$deg.casl.dist * nstats$main$md.main.casl

# 1G: concurrent
out$main$concurrent <- num * nstats$main$concurrent

# 1H: nodefactor("diag.status")
out$main$nodefactor_diag.status <- unname(table(out$attr$diag.status)) * nstats$main$nf.diag.status
...
out$casl$nodefactor_age.grp <- unname(table(out$attr$age.grp)) * nstats$casl$nf.age.grp
...
out$casl$nodefactor_deg.main <- num * nstats$casl$deg.main.dist * nstats$casl$md.casl.main

# 2G: concurrent
out$casl$concurrent <- num * nstats$casl$concurrent

# 2H: nodefactor("diag.status")
out$casl$nodefactor_diag.status <- unname(table(out$attr$diag.status)) * nstats$casl$nf.diag.status
...
out$inst$nodematch_age.grp <- sum(out$inst$edges *
                                    (unname(table(out$attr$age.grp))/num) * nstats$inst$nm.age.grp)

# 3C: nodefactor("age.grp")
out$inst$nodefactor_age.grp <- unname(table(out$attr$age.grp)) * nstats$inst$nf.age.grp
...
out$inst$nodefactor_risk.grp <- unname(table(out$attr$risk.grp)) * nstats$inst$nf.risk.grp

# 3G: nodefactor("deg.tot(3+)")

out$inst$nodefactor_deg.tot <- unname(table(out$attr$deg.tot)) * nstats$inst$nf.deg.tot

# 3H: nodefactor("diag.status")
out$inst$nodefactor_diag.status <- unname(table(out$attr$diag.status)) * nstats$inst$nf.diag.status
